...
def format_pub(args):
    ...
    n = [i for i in range(len(pub["authors"]))
         if "Rackham, B" in pub["authors"][i]][0]
    pub["authors"][n] = "\\textbf{Rackham, B. V.}"
    if len(pub["authors"]) > 5:
        fmt += ", ".join(pub["authors"][:4])
        fmt += ", \etal"
        if n >= 4:
            fmt += "\\ (including\\ \\textbf{Rackham, B. V.})"
...
def format_talk(args):
    ...
    n = [i for i in range(len(talk["authors"]))
         if "Rackha, B" in talk["authors"][i]][0]
    talk["authors"][n] = "\\textbf{Rackham, B. V.}"
...
if __name__ == "__main__":
    ...
    npapers = len(ref)
    nfirst = sum(1 for p in pubs if "Rackham, B" in p["authors"][0])
